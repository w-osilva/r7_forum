class CommentsController < ApplicationController
  ...
  # GET /comments.json
  def index
    @comments = params[:topic_id] ? Comment.where(topic_id: params[:topic_id]) : Comment.all
  end
  ...
  def create
    ...
    respond_to do |format|
      if @comment.save
        ...
        format.json { render :show, status: :created, location: @comment.topic }
      else
  ...
  def update
    respond_to do |format|
      if @comment.update(comment_params)
        ...
        format.json { render :show, status: :ok, location: @comment.topic }
      else
  ...
  private
    ...
    def comment_params
      params.require(:comment).permit(:text, :topic_id, :comment_id)
    end
end
